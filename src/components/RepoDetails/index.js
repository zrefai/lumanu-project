...
Details.IContainer = function DetailsRepoInfoContainer({
  children,
  ...otherProps
}) {
  return (
    <RepoInfoTextContainer {...otherProps}>{children}</RepoInfoTextContainer>
  );
};
...
Details.RContainer = function DetailsReleaseContainer({
  children,
  ...otherProps
}) {
  return (
    <ReleaseInfoContainer {...otherProps}>{children}</ReleaseInfoContainer>
  );
};
...
const RepoDetails = ({ repoID }) => {
  ...
  const renderDetails = () => {
      ...
      return (
        <Details>
          ...
          <div style={{ marginTop: "20px" }}>
            ...
            {repo.latest_release ? repo.latest_release.tag_name : "N/A"} on{" "}
            {repo.latest_release
              ? repo.latest_release.created_at.split("T")[0]
              : "N/A"}
          </div>
          <Details.RContainer>
            <ReactMarkdown>
              {repo.latest_release
                ...
                : "No release information for this repository"}
            </ReactMarkdown>
          </Details.RContainer>
        </Details>
      );
    ...
    return (
      <p>Please go back to the home page and select a repository to view</p>
    );
  };
