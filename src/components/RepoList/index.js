...
const RepoList = () => {
  ...
  const renderCard = () => {
    if (Object.keys(repos).length)
      return Object.keys(repos).map((key, i) => (
        <RepoCard key={i} repo={repos[key]} />
      ));
    return (
      <List.EmptyText>You are not tracking any repositories yet</List.EmptyText>
    );
  };

  return <List>{renderCard()}</List>;
};
